...
// Parse command with redirections and pipes
std::vector<Command> parseCommand(const std::vector<std::string>& tokens) {
    std::vector<Command> commands(1); // Start with one command
    size_t currentCmd = 0;

    for (size_t i = 0; i < tokens.size(); ++i) {
        const std::string& token = tokens[i];

        // Handle pipe - create a new command
        if (token == "|") {
        ...
        // Handle background process (only for the last command)
        if (token == "&" && i == tokens.size() - 1) {
            commands[currentCmd].backgroundProcess = true;
            continue;
        }

        // Handle input redirection
        if (token == "<" && i + 1 < tokens.size()) {
            commands[currentCmd].inputFile = tokens[++i];
            continue;
        }

        // Handle output redirection
        if (token == ">" && i + 1 < tokens.size()) {
            commands[currentCmd].outputFile = tokens[++i];
            commands[currentCmd].appendOutput = false;
            continue;
        }
        ...
        if (token == ">>" && i + 1 < tokens.size()) {
            commands[currentCmd].outputFile = tokens[++i];
            commands[currentCmd].appendOutput = true;
            continue;
        }

        // Handle error redirection
        if (token == "2>" && i + 1 < tokens.size()) {
            commands[currentCmd].errorFile = tokens[++i];
            commands[currentCmd].appendError = false;
            continue;
        }

        // Handle error redirection with append
        if (token == "2>>" && i + 1 < tokens.size()) {
            commands[currentCmd].errorFile = tokens[++i];
            commands[currentCmd].appendError = true;
            continue;
        }

        // Add as regular argument
        commands[currentCmd].args.push_back(token);
    }
    ...
    if (pathEnv) {
        ...
        while (std::getline(pathStream, path, ':')) {
            try {
                ...
                if (debugMode) std::cerr << "Debug: Could not read directory " << path << ": " << e.what() << std::endl;
            }
        }
    }

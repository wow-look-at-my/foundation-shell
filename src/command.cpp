...
Task<bool> Command::execute(int inputFd, int outputFd) const
{
	...
	else if (pid == 0)
	{
		...
		std::cerr << Colors::COLOR_RED << "Command not found: " << args[0] << Colors::COLOR_RESET << "\n";

		// Provide command suggestions if enabled
		if (shellConfig.enableCommandSuggestions)
		{
			std::vector<std::string> suggestions = findCommandSuggestions(args[0]);
...
// Parse command with redirections, pipes, and command chains
CommandChain parseCommandChain(const std::vector<std::string> &tokens)
{
	CommandChain chain;
	chain.commands.push_back(Command()); // Start with one command
	size_t currentCmd = 0;

	for (size_t i = 0; i < tokens.size(); ++i)
	{
		const std::string &token = tokens[i];
		TokenType tokenType = identifyToken(token, i == tokens.size() - 1);

		switch (tokenType)
		{
		case TokenType::Pipe:
			chain.operators.push_back(ChainOperator::Pipe);
			chain.commands.push_back(Command());
			currentCmd++;
			break;

		case TokenType::And:
			chain.operators.push_back(ChainOperator::And);
			chain.commands.push_back(Command());
			currentCmd++;
			break;

		case TokenType::Or:
			chain.operators.push_back(ChainOperator::Or);
			chain.commands.push_back(Command());
			currentCmd++;
			break;

		case TokenType::Background:
			chain.commands[currentCmd].backgroundProcess = true;
			break;

		case TokenType::RedirectStdIn:
			if (i + 1 < tokens.size())
			{
				chain.commands[currentCmd].inputFile = tokens[++i];
			}
			break;
		...
		case TokenType::RedirectStdOut:
			if (i + 1 < tokens.size())
			{
				chain.commands[currentCmd].outputFile = tokens[++i];
				chain.commands[currentCmd].appendOutput = false;
			}
			break;

		case TokenType::RedirectStdOutAppend:
			if (i + 1 < tokens.size())
			{
				chain.commands[currentCmd].outputFile = tokens[++i];
				chain.commands[currentCmd].appendOutput = true;
			}
			break;

		case TokenType::RedirectStdErr:
			if (i + 1 < tokens.size())
			{
				chain.commands[currentCmd].errorFile = tokens[++i];
				chain.commands[currentCmd].appendError = false;
			}
			break;

		case TokenType::RedirectStdErrAppend:
			if (i + 1 < tokens.size())
			{
				chain.commands[currentCmd].errorFile = tokens[++i];
				chain.commands[currentCmd].appendError = true;
			}
			break;

		case TokenType::Command:
		default:
			// Add as regular argument
			chain.commands[currentCmd].args.push_back(token);
			break;
		}
	}

	// Remove empty commands
	auto it = std::remove_if(chain.commands.begin(), chain.commands.end(),
							 [](const Command &cmd)
							 { return cmd.args.empty(); });

	// If we're removing commands, we need to adjust the operators vector accordingly
	if (it != chain.commands.end())
	{
		// Calculate indices of removed commands
		std::vector<size_t> removedIndices;
		for (auto cmd_it = it; cmd_it != chain.commands.end(); ++cmd_it)
		{
			removedIndices.push_back(std::distance(chain.commands.begin(), cmd_it));
		}

		// Remove corresponding operators
		if (!chain.operators.empty())
		{
			for (auto idx_it = removedIndices.rbegin(); idx_it != removedIndices.rend(); ++idx_it)
			{
				size_t idx = *idx_it;
				if (idx > 0 && idx - 1 < chain.operators.size())
				{
					chain.operators.erase(chain.operators.begin() + (idx - 1));
				}
			}
		}

		// Now remove the commands
		chain.commands.erase(it, chain.commands.end());
	}

	return chain;
}

// For backward compatibility - parse command with redirections and pipes
std::vector<Command> parseCommand(const std::vector<std::string> &tokens)
{
	// Use the new parsing function
	CommandChain chain = parseCommandChain(tokens);

	// For backward compatibility, convert back to a simple vector of commands
	// Note: This loses information about && and || operators

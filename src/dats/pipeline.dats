...
tests:
	...
	- desc: seq piped into head -2 terminates
	  cmd: "build/fsh-exec 'seq 1 100000 | head -2'"
	  timeout: 10
	  outputs:
		stdout:
			0: "^1$"
	...
	- desc: a failing producer reports the error but the pipeline exits 0
	  cmd: "build/fsh-exec 'nosuchcmd_fsh_test | cat'"
	  timeout: 10
	  outputs:
		stderr:
			- "nosuchcmd_fsh_test: command not found"
	...
	- desc: the pipeline status comes from the rightmost command
	  cmd: "build/fsh-exec \"true | false | sh -c 'exit 5'\""
	  exit: 5
	...
	- desc: data flows through a pipeline
	  cmd: "build/fsh-exec 'echo hello world | tr a-z A-Z'"
	  outputs:
		stdout:
			0: "^HELLO WORLD$"
	...
	- desc: output redirection overrides the pipe
	  cmd: "build/fsh-exec 'echo hi > {outputs.piped.txt} | cat'"
	  timeout: 10
	  outputs:
		!stdout:
			0: "."
	...
	- desc: input redirection inside a pipeline does not wedge the producer
	  cmd: "build/fsh-exec 'yes | cat < {inputs.side.txt}'"
	  timeout: 10
	  inputs:
		files:
			side.txt: |

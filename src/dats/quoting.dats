...
tests:
	...
	- desc: nested single quotes stay literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo 'outer 'inner' end'\""
	  outputs:
		stdout:
	...
	- desc: multi-level single-quote nesting
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo 'l1 'l2 'l3' l2' l1'\""
	  outputs:
		stdout:
			0: "^\\Ql1 'l2 'l3' l2' l1\\E$"
	...
	- desc: expansion still runs inside nested double quotes
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo \"path \"$HOME\" here\"'"
	  inputs:
		env:
			HOME: /home/testuser
	  outputs:
	...
	- desc: two adjacent quoted words are two arguments
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"printf '[%s]' 'a' 'b'\""
	  outputs:
		stdout:
			0: "^\\Q[a][b]\\E$"
	...
	- desc: the escaped-apostrophe dance produces one word
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo 'don'\\''t'\""
	  outputs:
		stdout:
			0: "^\\Qdon't\\E$"
	...
	- desc: an unreturned single-quote nesting depth is a loud error
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo 'hello 'world\""
	  exit: 1
	  outputs:
		!stdout:
			0: "."
	...
	- desc: an unreturned double-quote nesting depth is a loud error
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo \"Total: \"$N'"
	  exit: 1
	  outputs:
		!stdout:
			0: "."
	...
	- desc: whole-token suppression keeps $HOME literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo 'a'\\$HOME\""
	  outputs:
		stdout:
			0: "^\\Qa$HOME\\E$"
	...
	- desc: double quotes inside single quotes are literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo 'say \\\"hello\\\"'\""
	  outputs:
		stdout:
			0: "^\\Qsay \"hello\"\\E$"

	- desc: a single quote inside double quotes is literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo \\\"it's fine\\\"\""
	  outputs:
		stdout:
			0: "^\\Qit's fine\\E$"
	...
	- desc: escaped quotes inside double quotes
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo \"say \\\"hi\\\"\"'"
	  outputs:
		stdout:
			0: "^\\Qsay \"hi\"\\E$"
	...
	- desc: a backslash is literal inside single quotes
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"echo 'a\\nb'\""
	  outputs:
		stdout:
			0: "^\\Qa\\nb\\E$"

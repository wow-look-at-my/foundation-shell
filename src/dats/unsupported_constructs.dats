...
tests:
	...
	- desc: background & fails fast instead of hanging the shell
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c \"bash -c 'sleep 30' &\""
	  timeout: 5
	  exit: 1
	  outputs:
		stderr:
	...
	- desc: background & does not swallow the rest of the line
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'echo a & echo b' 2> {outputs.err.txt}"
	  exit: 1
	  outputs:
		!stdout:
			0: "."
	...
	- desc: a here-document is named as unsupported, not as a missing target
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c \"$(cat {inputs.line.fsh})\""
	  exit: 1
	  inputs:
		files:
			line.fsh: "cat <<EOF"
	...
	- desc: a here-string reports exactly one error
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c \"$(cat {inputs.line.fsh})\" 2> {outputs.err.txt}"
	  exit: 1
	  inputs:
		files:
			line.fsh: "cat <<<word"
	...
	- desc: fd duplication is still rejected
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'echo hi 2>&1'"
	  exit: 1
	  outputs:
		stderr:
	...
	- desc: an ampersand inside a word is still literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'echo a&b'"
	  outputs:
		stdout:
			0: "^\\Qa&b\\E$"
	...
	- desc: an escaped ampersand is still literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'echo x\\&'"
	  outputs:
		stdout:
			0: "^\\Qx&\\E$"

	- desc: a url query string survives
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'echo \"http://h/p?a=1&b=2\"'"
	  outputs:
		stdout:
			0: "^\\Qhttp://h/p?a=1&b=2\\E$"

	- desc: the && operator is unaffected
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'true && echo ok'"
	  outputs:
		stdout:
			0: "^ok$"
	...
	- desc: capture-then-use is rejected instead of yielding empty
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'OUT=$(echo captured); echo $OUT'"
	  exit: 1
	  outputs:
		!stdout:
			0: "."
	...
	- desc: export then use in the same input is rejected
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'export X=hi; echo $X'"
	  exit: 1
	  outputs:
		stderr:
	...
	- desc: an assignment is still visible to a child that reads the environment
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'FSH_ENVCHECK=hi; printenv FSH_ENVCHECK'"
	  outputs:
		stdout:
			0: "^hi$"

	- desc: a single-quoted reference is data and still resolves in the child
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c \"FSH_CHILD=hi; sh -c 'echo \\$FSH_CHILD'\""
	  outputs:
		stdout:
			0: "^hi$"

	- desc: an assignment with no later expansion is fine
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'FSH_UNUSED=hi; echo done'"
	  outputs:
		stdout:
			0: "^done$"

	- desc: expanding a variable nobody assigned here is fine
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'FSH_A=1; echo [$FSH_B]'"
	  outputs:
		stdout:
			0: "^\\Q[]\\E$"

	- desc: a single < input redirection is unaffected
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" -c 'cat < {inputs.in.txt}'"
	  inputs:
		files:
			in.txt: |
				payload

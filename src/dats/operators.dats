...
tests:
	...
	- desc: skip propagation makes false && echo a && echo b silent
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'false && echo a && echo b'"
	  exit: 1
	  outputs:
		!stdout:
			- "a"
	...
	- desc: skip propagation makes true || echo a || echo b silent
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'true || echo a || echo b'"
	  outputs:
		!stdout:
			- "a"
			- "b"
	...
	- desc: false || echo one && echo two prints both
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'false || echo one && echo two'"
	  outputs:
		stdout:
			0: "^one$"
			1: "^two$"
	...
	- desc: true && false || echo fallback prints fallback
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'true && false || echo fallback'"
	  outputs:
		stdout:
			0: "^fallback$"
	...
	- desc: a spawn failure recovers through ||
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'nosuchcmd_fsh_test || echo fallback'"
	  outputs:
		stdout:
			0: "^fallback$"
		stderr:
	...
	- desc: a spawn failure does not stop a semicolon chain
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'nosuchcmd_fsh_test ; echo next'"
	  outputs:
		stdout:
			0: "^next$"
	...
	- desc: a trailing semicolon is valid
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo hi ;'"
	  outputs:
		stdout:
			0: "^hi$"
	...
	- desc: consecutive operators are a parse error
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo a ; ; echo b'"
	  exit: 1
	  outputs:
		!stdout:
			- "a"
	...
	- desc: a leading operator is a parse error
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" '| cat'"
	  exit: 1
	  outputs:
		stderr:
	...
	- desc: a quoted redirection character does not truncate grep's input
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"grep '>' {inputs.gfile}\" && wc -l < {inputs.gfile}"
	  inputs:
		files:
			gfile: |
				a>b
	...
	- desc: a pipe works without surrounding whitespace
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo hello|tr a-z A-Z'"
	  outputs:
		stdout:
			0: "^HELLO$"
	...
	- desc: a pipe binds tighter than &&
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo test | grep test && echo found'"
	  outputs:
		stdout:
			0: "^test$"
			1: "^found$"

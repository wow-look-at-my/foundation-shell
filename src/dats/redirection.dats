...
tests:
	...
	- desc: the > operator truncates the target file
	  cmd: "build/fsh-exec 'echo first > {outputs.out.txt}' && build/fsh-exec 'echo second > {outputs.out.txt}'"
	  outputs:
		files:
			out.txt:
				match:
	...
	- desc: the >> operator appends to the target file
	  cmd: "build/fsh-exec 'echo line1 > {outputs.log.txt}' && build/fsh-exec 'echo line2 >> {outputs.log.txt}'"
	  outputs:
		files:
			log.txt:
				match:
	...
	- desc: the < operator redirects input
	  cmd: "build/fsh-exec 'cat < {inputs.in.txt}'"
	  inputs:
		files:
			in.txt: |
				from-file
	...
	- desc: the 2> operator captures stderr
	  cmd: "build/fsh-exec 'ls /nonexistent_fsh_dir 2> {outputs.err.txt}'; test $? -ne 0"
	  outputs:
		!stdout:
			0: "."
		files:
	...
	- desc: the 2>> operator appends stderr
	  cmd: "build/fsh-exec 'ls /nonexistent_fsh_dir 2> {outputs.err.txt}'; build/fsh-exec 'ls /nonexistent_fsh_dir 2>> {outputs.err.txt}'; wc -l < {outputs.err.txt}"
	  outputs:
		stdout:
			0: "^ *2$"
	...
	- desc: the last redirection wins and the earlier target is never created
	  cmd: "build/fsh-exec 'echo test > {outputs.f1.txt} > {outputs.f2.txt}'"
	  outputs:
		files:
			f2.txt:
				match:
	...
	- desc: an empty redirection target is a parse error
	  cmd: "build/fsh-exec 'echo hi > $UNSET_FSH_VAR'"
	  exit: 1
	  outputs:
		!stdout:
			0: "."
	...
	- desc: 2>&1 is rejected with the fd-duplication error
	  cmd: "build/fsh-exec 'echo hi 2>&1'"
	  exit: 1
	  outputs:
		!stdout:
			0: "."
	...
	- desc: an expansion-produced &1 target is a legal filename
	  cmd: "F=\"$PWD/build/fsh-exec\"; cd \"$(mktemp -d)\" && \"$F\" 'echo hi > $X' && cat '&1'"
	  inputs:
		env:
			X: "&1"
	  outputs:
	...
	- desc: the input open failure message is canonical
	  cmd: "build/fsh-exec 'cat < nonexistent.txt'"
	  exit: 1
	  outputs:
		stderr:
	...
	- desc: the chain continues after an input open failure
	  cmd: "build/fsh-exec 'cat < nonexistent.txt || echo recovered'"
	  outputs:
		stdout:
			0: "^recovered$"
		stderr:
	...
	- desc: the output open failure message is canonical
	  cmd: "build/fsh-exec 'echo x > missingdir/file.txt'"
	  exit: 1
	  outputs:
		stderr:
	...
	- desc: redirection position is independent
	  cmd: "build/fsh-exec '< {inputs.in.txt} cat'"
	  inputs:
		files:
			in.txt: |
				data
	...
	- desc: the pwd builtin respects output redirection
	  cmd: "build/fsh-exec 'pwd > {outputs.where.txt}' && [ \"$(cat {outputs.where.txt})\" = \"$PWD\" ]"

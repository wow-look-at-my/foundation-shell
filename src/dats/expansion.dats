...
tests:
	...
	- desc: an unset variable stays as an empty argument
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'printf [%s] a $FSH_UNSET_VAR b'"
	  outputs:
		stdout:
			0: "^\\Q[a][][b]\\E$"
	...
	- desc: no post-expansion word splitting
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'printf [%s] $X'"
	  inputs:
		env:
			X: "a b"
	  outputs:
	...
	- desc: ${?} is a verbatim environment lookup, so it is empty
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'printf [%s] ${?}'"
	  outputs:
		stdout:
			0: "^\\Q[]\\E$"
	...
	- desc: $1 is literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo $1'"
	  outputs:
		stdout:
			0: "^\\Q$1\\E$"
	...
	- desc: an escaped dollar is literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo \\$HOME'"
	  outputs:
		stdout:
			0: "^\\Q$HOME\\E$"
	...
	- desc: variable names are ASCII-only
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo $FSH_A\u00e9'"
	  inputs:
		env:
			FSH_A: x
	  outputs:
	...
	- desc: greedy name matching
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo $ABC'"
	  inputs:
		env:
			A: a
			AB: ab
	...
	- desc: braces limit the variable name
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo ${A}BC'"
	  inputs:
		env:
			A: a
	  outputs:
	...
	- desc: an unclosed brace stays literal
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo ${FSH_UNCLOSED'"
	  outputs:
		stdout:
			0: "^\\Q${FSH_UNCLOSED\\E$"
	...
	- desc: a value holding $(...) is data, not code
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo $X'"
	  inputs:
		env:
			X: "$(echo pwned)"
	  outputs:
	...
	- desc: a value holding backticks is data, not code
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo $Y'"
	  inputs:
		env:
			Y: "`date`"
	  outputs:
	...
	- desc: a value holding $HOME is data, not re-expanded
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo $Z'"
	  inputs:
		env:
			Z: "$HOME"
	  outputs:
	...
	- desc: a missing braced variable expands to nothing inside a word
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo prefix${MISSING_FSH_VAR}end'"
	  outputs:
		stdout:
			0: "^prefixend$"
	...
	- desc: tilde-user does not expand
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo ~user'"
	  outputs:
		stdout:
			0: "^~user$"
	...
	- desc: a tilde inside a token does not expand
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo /path/~'"
	  outputs:
		stdout:
			0: "^\\Q/path/~\\E$"
	...
	- desc: tilde suppression is whole-token
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo ~/\"docs\"'"
	  inputs:
		env:
			HOME: /home/testuser
	  outputs:
	...
	- desc: double quotes allow variable expansion
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'echo \"$FSH_V\"'"
	  inputs:
		env:
			FSH_V: hello
	  outputs:

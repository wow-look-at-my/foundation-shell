...
tests:
	...
	- desc: cd changes the directory for the rest of the chain
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'cd / ; pwd'"
	  outputs:
		stdout:
			0: "^/$"
	...
	- desc: cd failure message and status
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'cd /nonexistent_fsh_dir'"
	  exit: 1
	  outputs:
		stderr:
	...
	- desc: exit inside a pipeline does not stop the shell
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'exit 7 | cat ; echo after'"
	  outputs:
		stdout:
			0: "^after$"
	...
	- desc: exit with a non-numeric argument is a usage error
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'exit abc'"
	  exit: 2
	  outputs:
		stderr:
	...
	- desc: the shell survives a non-numeric exit argument
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'exit abc ; echo survived'"
	  outputs:
		stdout:
			0: "^survived$"
	...
	- desc: command not found is 127 with the canonical message
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'nosuchcmd_fsh_test'"
	  exit: 127
	  outputs:
		stderr:
	...
	- desc: a non-executable file is 126
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" {inputs.notexec.sh}"
	  exit: 126
	  inputs:
		files:
			notexec.sh: |
	...
	- desc: signal death reports 128+N
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"sh -c 'kill -9 \\$\\$'\""
	  exit: 137

	# execution.md export: sets the variable; children see it.
	- desc: export sets a variable that children see
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"export FSH_TEST_EXPORT=bar ; sh -c 'echo \\$FSH_TEST_EXPORT'\""
	  outputs:
		stdout:
			0: "^bar$"
	...
	- desc: export with no arguments prints the environment
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'export FSH_TEST_PRINTED=hello ; export'"
	  outputs:
		stdout:
	...
	- desc: export continues past invalid names
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"export FSH_A=1 1BAD=2 FSH_B=3 ; sh -c 'echo \\$FSH_A\\$FSH_B'\""
	  outputs:
		stdout:
			0: "^13$"
		stderr:
	...
	- desc: export of an invalid name alone exits 1
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'export 1BAD=2'"
	  exit: 1
	...
	- desc: a standalone assignment is visible to children
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"FSH_TEST_ASSIGN=baz ; sh -c 'echo \\$FSH_TEST_ASSIGN'\""
	  outputs:
		stdout:
			0: "^baz$"
	...
	- desc: a single-quoted FOO=bar is a command, not an assignment
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" \"'FSH_Q=bar'\""
	  exit: 127
	  outputs:
		stderr:
	...
	- desc: a prefix assignment VAR=x cmd is not supported
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" 'FSH_P=x true'"
	  exit: 127
	  outputs:
		stderr:
	...
	- desc: help lists export and $? and no background operator
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" help"
	  outputs:
		stdout:
			- "export"
			- "$?"
	...
	- desc: clear writes the ANSI clear sequence
	  cmd: "\"$GO_TOOLCHAIN_DATS_BUILD_DIR/fsh-exec\" clear"
	  outputs:
		stdout:
			0: "^\\x1b\\[2J\\x1b\\[H$"
